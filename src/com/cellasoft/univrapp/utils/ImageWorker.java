...
public abstract class ImageWorker {
	...
	public Bitmap get(String key) {
		...
		synchronized (mImageCache) {
			BitmapDrawable bitmap = mImageCache.getBitmapFromMemCache(key);
			if (bitmap == null)
	 ...
	 * been set using {@link ImageWorker#addImageCache}. If the image is found
	...
	public static void cancelWork(ImageView imageView) {
		...
		if (bitmapWorkerTask != null) {
			...
			if (BuildConfig.DEBUG) {
				LOGD(TAG, "cancelWork - cancelled work for "
	...
	public static boolean cancelPotentialWork(Object data, ImageView imageView) {
		...
		if (bitmapWorkerTask != null) {
			...
			if (bitmapData == null || !bitmapData.equals(data)) {
				bitmapWorkerTask.cancel(true);
				if (BuildConfig.DEBUG) {
					LOGD(TAG, "cancelPotentialWork - cancelled work for "
							+ data);
				}
			} else {
